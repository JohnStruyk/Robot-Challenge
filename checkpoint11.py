...
def get_all_cube_transforms(observation, camera_intrinsic, camera_pose):
    """
    Same pipeline as get_transform_cube, but returns ALL candidate cube poses.
    """
...
def run_pure_vision_perception(cv_image, point_cloud, camera_intrinsic):
    ...
    try:
        all_results, seg_msg = get_all_cube_transforms(
        ...
        if not all_results:
            ...
            return None, None, disp, seg_msg

        # keep original behavior: pick ONE cube (first)
        t_robot_cube, t_cam_cube = all_results[0]

        lines = [
            f"Segmentation: {seg_msg}",
            f"Detected cubes: {len(all_results)}",
        ...
        # draw ALL cube poses
        for (_tr, t_cam) in all_results:
            if t_cam is not None and numpy.isfinite(t_cam).all():
                draw_pose_axes(disp, camera_intrinsic, t_cam)
